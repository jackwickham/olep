...
public class RandomStockFactory implements StockFactory {
    private int nextItemId;
    private RandomDataGenerator rand;
    private int warehouseId;

    private static Map<Integer, RandomStockFactory> instances = new HashMap<>();

    private RandomStockFactory(WarehouseShared warehouse) {
        nextItemId = 1;
        rand = new RandomDataGenerator();
        warehouseId = warehouse.id;
    }

    public static RandomStockFactory instanceFor(WarehouseShared warehouse) {
        if (!instances.containsKey(warehouse.id)) {
            instances.put(warehouse.id, new RandomStockFactory(warehouse));
        }
